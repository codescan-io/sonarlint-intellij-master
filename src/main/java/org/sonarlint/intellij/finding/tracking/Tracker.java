...
public class Tracker<RAW extends Trackable, BASE extends Trackable> {
  ...
  public Tracking<RAW, BASE> track(Input<RAW> rawInput, Input<BASE> baseInput) {
    return ApplicationManager.getApplication().<Tracking<RAW, BASE>>runReadAction(() -> {
      ...
      match(tracking, ServerFindingSearchKeyFactory.INSTANCE);
  ...
  private static class ServerFindingSearchKey implements SearchKey {
    private final String serverFindingKey;

    ServerFindingSearchKey(Trackable trackable) {
      serverFindingKey = trackable.getServerFindingKey();
    }
    ...
    public boolean equals(Object o) {
      ...
      var that = (ServerFindingSearchKey) o;

      return !SonarLintUtils.isBlank(serverFindingKey) && !SonarLintUtils.isBlank(that.serverFindingKey) && serverFindingKey.equals(that.serverFindingKey);
    }

    @Override
    public int hashCode() {
      return serverFindingKey != null ? serverFindingKey.hashCode() : 0;
    }
  }

  private enum ServerFindingSearchKeyFactory implements SearchKeyFactory {
    INSTANCE;

    @Override
    public SearchKey apply(Trackable trackable) {
      return new ServerFindingSearchKey(trackable);
    }
  }
}
